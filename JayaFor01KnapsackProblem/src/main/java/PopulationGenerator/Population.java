...
public class Population {
    ...
    public double[][] GeneratePopulation() {
        Supplier<double[]> generateRandomly = () -> IntStream.range(0, PopulationStore[0].length).map(i -> ThreadLocalRandom.current().nextInt(2)).asDoubleStream().toArray();
        IntStream.range(0, PopulationStore.length).forEach(i -> PopulationStore[i] = generateRandomly.get());
     ...
     */
    public void DisplayPopulation(double[][] Population) {
